...
[[ -z $CHAIN_ID ]] && CHAIN_ID="canto_7700-1"
[[ -z $CONFIG_PATH ]] && CONFIG_PATH="/root/.cantod"
[[ -z $PUBLIC_RPC ]] && PUBLIC_RPC="https://canto-rpc.polkachu.com:443"
...
init_node() {
  ...
  wget -O $CONFIG_PATH/config/genesis.json ${GENESIS_URL:-https://github.com/Canto-Network/Canto/raw/genesis/Networks/Mainnet/genesis.json}
  ...
  sed -i \
    ...
    -e 's|^minimum-gas-prices =.*|minimum-gas-prices = "0.0025acanto"|' \
    $CONFIG_PATH/config/app.toml
}
...
create_account() {
  ...
  expect -c "
      ...
      spawn $BIN keys add $WALLET ${KEYRING_BACKEND:+--keyring-backend $KEYRING_BACKEND} --home $CONFIG_PATH
      exp_internal 0
