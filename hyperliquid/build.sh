...
echo "Which Hyperliquid network should the image be built for?"
PS3="Hyperliquid network: "
select network in "Mainnet" "Testnet"
do
    case $network in
        "Mainnet")
            HL_CHAIN="Mainnet"; break
            ;;
        "Testnet")
            HL_CHAIN="Testnet"; break
            ;;
    esac
done

IMAGE_TAG="${BUILD_DATE//-/}-${HL_CHAIN,,}"
...
echo -e "HL chain: \t$HL_CHAIN\n"

docker build -f "$DOCKERFILE" "$DIR" \
     --build-arg IMAGE_TAG="$IMAGE_TAG" \
     --build-arg HL_CHAIN="$HL_CHAIN" \
     --tag "$IMAGE"
