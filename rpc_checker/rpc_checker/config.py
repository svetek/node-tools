...
CHAINS = ('NEAR', 'NEART', 'ETH1', 'SEP1', 'HOL1', 'BASE', 'BASES',
          'ARBITRUM', 'ARBITRUMN', 'ARBITRUMS')


def validate_url(url, schemes):
    ...
    try:
        p = urlsplit(url)
        port = p.port
        if (p.scheme not in schemes or not p.hostname or p.fragment
                or p.username is not None or p.password is not None or port == 0
                or '\\' in p.netloc):
            raise ValueError
        url.encode('ascii')
...
class Config:
    ...
    def from_env(cls):
        ...
        for name, value in raw.items():
            ...
            if not isinstance(addons, list) or any(not isinstance(a,str) for a in addons) or not isinstance(ws,str):
        ...
        defaults = {'NEAR':'https://rpc.mainnet.near.org', 'NEART':'https://rpc.testnet.near.org',
                    'BASE':'https://mainnet.base.org', 'BASES':'https://sepolia.base.org'}
        ...
        return cls(chain_id, nodes, trusted, **values, retries=retries,
                   host=os.getenv('HTTP_HOST', '0.0.0.0'), port=port, workers=workers)
