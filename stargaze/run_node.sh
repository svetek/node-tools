...
[[ -z $CHAIN_ID ]] && CHAIN_ID="stargaze-1"
[[ -z $CONFIG_PATH ]] && CONFIG_PATH="/root/.starsd"
[[ -z $PUBLIC_RPC ]] && PUBLIC_RPC="https://stargaze-rpc.polkachu.com:443"
...
init_node() {
  ...
  wget -O $CONFIG_PATH/config/genesis.json ${GENESIS_URL:-https://snapshots.polkachu.com/genesis/stargaze/genesis.json}
  ...
  $BIN config broadcast-mode "sync" --home $CONFIG_PATH
  ...
  sed -i \
    ...
    -e 's|^minimum-gas-prices =.*|minimum-gas-prices = "1ustars"|' \
    $CONFIG_PATH/config/app.toml
}
